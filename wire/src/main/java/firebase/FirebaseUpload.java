...
public class FirebaseUpload {
	...
	public static void fileInfoUpload(String line, String status) {
		try {
			...
			// 가공파일 이름 업로드
			if (status == "finished") {
				FirebaseUploadForm.uploadForm(pathInfo, "file", NcFile.getNcFile(), "status", statusSaved, "wholeProcessingTime", ProcessingTime.getWholeProcessingTime(), "actualProcessingTime", ProcessingTime.getActualProcessingTime(), "time", CurrentTime.getTime("date") + " " + CurrentTime.getTime("time"));
			} else {
				FirebaseUploadForm.uploadForm(pathInfo, "file", NcFile.getNcFile(line), "status", statusSaved, "time", CurrentTime.getTime("date") + " " + CurrentTime.getTime("time"));
			}
