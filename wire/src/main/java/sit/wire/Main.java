    	...
    	try {
	    	while (true) {
	    			...
	    			for (String line : addedLines) {  // 추가된 로그 탐색
	    				...
	    				for (String logKey : LogData.getLogKeyList()) {  // 추가된 로그에 특정 key포함되어있는지 확인
	    					if (!readFirstLog && line.contains(logKey)) {
	    						...
	    						// 실제 가공 시간 측정
	    						ProcessingTime.calculateTime(logKey);
	    				...
	    				if (!search && line.contains("Nc File:")) { 
	    					FirebaseUpload.fileInfoUpload(line, "start");
	    					ProcessingTime.calculateTime("start");
	    					search = true;
	    					reseted = false;
	    				
	    				// reset여부 확인
	    				} else if (!search && !reseted && line.contains("Reset")) { 
	    					FirebaseUpload.fileInfoUpload(line, "finished");
	    					search = true;
	    					reseted = true;
	    		...
	    		if (!ManagingStatus.getProcessingStatus() && ScreenDetection.Restart()) {
	    			...
	    			ProcessingTime.calculateTime("가공감지");
	    		}
	    		
	    		
	        	TimeSleep.sleep(0.1f);  // 시간 지연
