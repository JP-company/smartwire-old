...
def firebase(data, coment): # 서버 연결
    wire_off = db.collection(u'wire_2').document(u'dates').collection(u'%s'%date).document(u'%s'%now)
    wire_off.set({
        u'now': u'%s'%now,
        u'onoff': u'%s'%data
    })
    wire_off_date = db.collection(u'wire_2').document(u'%s'%date)
    wire_off_date.set({
        u'date': u'%s'%date,
    })
    wire_push = db.collection(u'wire_2').document(u'dates').collection('time').document(u'%s %s'%(date, now))
    wire_push.set({
        u'push' : u'yes'
    })
