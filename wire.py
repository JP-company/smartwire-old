...
idf.RGBCollector(model, "uncut")
idf.RGBCollector(model, "finished")
...
def uncut_reader(): # 줄 씹힘 **KM에는 해당 X
...
def pause_reader(): # 와이어 미동작 **km에는 해당 x
    screen = ImageGrab.grab()
    if model == "wire_1" or model == "wire_2":
        pos = (1120, 315)
        RGB = screen.getpixel(pos)
...
def moff_reader(): # M코드 정지 **다름
    ...
    if RGB == red or RGB == black:
        return True

def start_reader(): # 초록불 감지 센서 **다름
    screen = ImageGrab.grab()
    if model == "wire_1" or model == "wire_2":
        pos_1 = (765,700)
        pos_2 = (600, 760)
    ...
    if RGB_1 == green and RGB_3 == green and RGB_4 == green and RGB_5 == green:
        return True
...
if start_reader() == True: # 가동중일때
    while True:
        while True:
            ...
            if start_reader() == True: # 최초 초록불 확인, 퇴근 후
                firebase('on', '가공 시작')
                break
        while True:
            time.sleep(10)
            if start_reader() != True: # 최초 회색불 감지, 퇴근 후
